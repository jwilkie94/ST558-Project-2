...
To begin, we read in the data set, then created a subset of the data to look at only the news article type that we are interested in.  The "URL" variable was removed because it was not useful to creating a model.  
...
data<-online_news_popularity%>%subset(select=-url)%>%filter(data_channel_is_bus == 1)
...
Next we can look at how keywords can affect the number of shares for an article.  The scatter plot below shows the kw_avg_avg plotted against shares.  The correlations that are included on this plot and can be interpreted in the same way as stated above.  
...
We think the global rate positive words and negative words would have some impact on shares. So we plot scatter plots. The correlation between global rate positive words and negative words and shares is 0.016 and 0.0086, which means the global rate positive and negative words barely have influence on people's sharing an article.  

```{r global positive words}
correlation_global_pos<-cor(data$global_rate_positive_words, data$shares)
correlation_global_pos
ggplot(data, aes(x=global_rate_positive_words, y=shares))+geom_point(color="green")+labs(x="global_rate_positive_words", y="Shares" ,title="global_rate_positive_words vs. Shares")+geom_text(x=0.15, y=7e+05, size=8,label = paste0("Correlation =", round(correlation_global_pos,2)))
```

```{r global negative words}
correlation_global_neg<-cor(data$global_rate_negative_words, data$shares)
correlation_global_neg
ggplot(data, aes(x=global_rate_negative_words, y=shares))+geom_point(color="blue")+labs(x="global_rate_negative_words", y="Shares" ,title="global_rate_negative_words vs. Shares")+geom_text(x=0.2, y=10e+05, size=8, label = paste0("Correlation = " , round(correlation_global_neg,2)))
```  



Also, we inspect the impact of rate positive words and negative words on shares. The result shows the rate negative words has higher correlation with shares than that of rate positive words, but both parameters had on strong linear relationship with shares.

```{r rate postive words}
correlation_rate_pos<-cor(data$rate_positive_words, data$shares)
correlation_rate_pos
ggplot(data, aes(x=rate_positive_words, y=shares))+geom_point(color="yellow")+labs(x="rate_positive_words", y="Shares" ,title="rate_positive_words vs. Shares")+geom_text(x=0.22, y=7e+05, size=8,label = paste0("Correlation =", round(correlation_rate_pos,2)))

correlation_rate_neg<-cor(data$rate_negative_words, data$shares)
correlation_rate_neg
ggplot(data, aes(x=rate_negative_words, y=shares))+geom_point(color="orange")+labs(x="rate_negative_words", y="Shares" ,title="rate_negative_words vs. Shares")+geom_text(x=0.22, y=7e+05, size=8,label = paste0("Correlation =", round(correlation_rate_neg,2)))

```  



We are interested in influence of number of images and videos on shares. Based on the result, we can see shares of articles actually decreased with the number of images and videos went up. 

```{r images and vedios}
correlation_imgs<-cor(data$num_imgs, data$shares)
correlation_imgs
ggplot(data, aes(x=num_imgs, y=shares))+geom_point(color="red")+labs(x="num_imgs", y="Shares" ,title="num_imgs vs. Shares")+geom_text(x=0.22, y=7e+05, size=2,label = paste0("Correlation =", round(correlation_imgs,2)))

correlation_videos<-cor(data$num_videos, data$shares)
correlation_videos
ggplot(data, aes(x=num_videos, y=shares))+geom_point(color="purple")+labs(x="num_videos", y="Shares" ,title="num_videos vs. Shares")+geom_text(x=0.22, y=7e+05, size=2,label = paste0("Correlation =", round(correlation_videos,2)))
```  
...
Now each of the models will be fitted using the training data set and analyzed through 10 fold cross validation.  The test set will be used for prediction.
...
linear_fit_1 <- train(shares~self_reference_avg_sharess+abs_title_sentiment_polarity+abs_title_subjectivity+avg_positive_polarity+avg_negative_polarity+is_weekend+rate_positive_words+rate_negative_words, data=training, method = "lm", trControl=trainControl(method="cv", number=10),preProcess = c("center", "scale"))
...
linear_fit_2 <- train(shares~global_rate_negative_words+num_imgs+ global_rate_negative_words:num_imgs+I(global_rate_negative_words^2)+I(num_imgs^2), data=training, method = "lm", trControl=trainControl(method="cv", number=10),preProcess = c("center", "scale"))
...
Now that all of the models are created and chosen using cross validation, we can use the **predict** function on the test set to find the best predictor from our chosen models.   
...
paste0("The best predictor model of the ones we've created was selected by choosing the model with the lowest RMSE value.  For this data subset, the best model is ", model,".")
```