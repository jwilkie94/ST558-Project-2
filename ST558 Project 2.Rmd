...
To begin, we read in the data set, then created a subset of the data to look at only the news article type that we are interested in.  The "URL" variable was removed because it was not useful to creating a model. A variable named "channel" was added to allow us to automate R markdown documents based on article type.   
...
We can also look at how keywords can affect the number of shares for an article.  The scatter plot below shows the kw_avg_avg plotted against shares.  The correlations that are included on this plot and can be interpreted in the same way as stated above.  
