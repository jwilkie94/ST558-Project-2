...
linear_fit_1 <- train(shares~weekday_is_monday+weekday_is_tuesday+weekday_is_wednesday+weekday_is_thursday+weekday_is_friday+global_rate_positive_words:global_rate_negative_words+num_imgs+num_keywords+n_unique_tokens, data=training, method = "lm", trControl=trainControl(method="cv", number=10),preProcess = c("center", "scale"))
...
A random forest model creates multiple trees from bootstrap analysis then averages the results.  Random forest models randomly select a subset of predictors for averaging, so the results won't be affected by one or two very strong predictors.  Below is a random forest model fitted from the data.  The model includes the all predictors except for data channel, which cannot be used to predict the response as the data is filtered for the channel.  

```{r rf}
set.seed(89)
random_forest_fit <- train(shares~.-data_channel_is_lifestyle-data_channel_is_entertainment-data_channel_is_bus-data_channel_is_socmed-data_channel_is_tech-data_channel_is_world, data=training, method = "rf", trControl=trainControl(method="cv", number=10),preProcess = c("center", "scale"), tuneGrid=data.frame(mtry=(1:10)))
...
postResample(pred_linear_1,test$shares)
postResample(pred_linear_2,test$shares)
postResample(random_forest_fit,test$shares)
postResample(boosted_tree_fit,test$shares)
```